...
<p class="metadata">Date de mise à jour : 10/09/26 16:24</p>
...
| `world/info.py`            | `full`, `boats`, `cumulative`, `hall_of_fame`, `leaders`, `metadata`, `plots`, `snapshot`                                           |
...
- `actor … stats` rend des valeurs déjà agrégées : le socle de l'espèce, les gènes de la sous-espèce, les traits de sous-espèce, de créature et de clan, l'équipement, la progression acquise au fil des conversations et de l'âge, puis les bonus de niveau.
...
- `world … hall_of_fame` pèse **tous les corps du monde, bêtes comprises** — le seul podium qui traverse les espèces, là où `ranks_in_species` ne compare un corps qu'aux siens.
- Les stats d'un mineur sont **bridées** : `damage_max` et `health_max` valent la moitié tant que `adult_age` n'est pas atteint — ce champ ne sort que là. La valeur adulte ne s'en déduit pas : le corps porte ses traits propres, et la base de sa lignée (`subspecies … stats`) n'est pas la sienne.
