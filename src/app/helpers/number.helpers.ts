...
export class NumberHelpers {
  ...
  public static compact = (value: number): string => this.isCompacted(value) ? `${Number((value / 1000).toFixed(1))} K` : String(Math.round(value * 10) / 10);
  ...
  public static ordinal(rank: number, isFrench: boolean): string {
    ...
    return rank % 100 >= 11 && rank % 100 <= 13 ? 'th' : ({ 1: 'st', 2: 'nd', 3: 'rd' }[rank % 10] ?? 'th');
  }
