...
import { CHAPTER_REGISTRY, CITY_REGISTRY, KINGDOM_REGISTRY, PERSON_REGISTRY, SAVES_DIR } from '../constants';
...
export class RegistryService {
  ...
  public readonly chapter = signal(''); // Current chapter slug — components build per-chapter asset paths (crowns) from it.
  ...
  public load(slug: string): Observable<unknown> {
    CHAPTER_REGISTRY.slug = slug;
    this.chapter.set(slug);
    return forkJoin([
